...
from storage import load_sleep, get_recent_memory

LOW_SLEEP = 6.0                 # under this counts as a short night
MIN_PAIRS = 3                   # paired sleep+mood days needed before claiming a pattern
_POSITIVE = {"good", "neutral"} # joyful / calm: intense = lighter, not heavier
...
def _mood_weight_by_date() -> dict:
    out: dict = {}
    try:
        entries = get_recent_memory(days=30)
    except Exception:
        entries = []
    for e in entries:
        if e.get("type") != "mood":
...
def detect_sleep_mood_pattern() -> str | None:
    ...
    if diff >= 1.0:
        return (
            f"On nights under {int(LOW_SLEEP)} hours of sleep, your days have landed about "
            f"{diff} points heavier than after a fuller night. Sleep looks like a real lever for you."
        )
    if diff <= -1.0:
        return (
            "Interestingly, your heavier days haven't tracked with short sleep — "
            "something else may be driving them. Worth noticing together."
        )
    return None
...
def vitals_note() -> str:
    ...
    try:
        ...
        return (
            "\nVitals: " + ", ".join(facts) + "." + extra +
            " If it feels natural, you may gently connect how they're feeling to their sleep — "
            "but reference only these real figures; invent nothing."
        )
    except Exception:
        return ""
