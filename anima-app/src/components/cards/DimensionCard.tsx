...
export function DimensionCard({
  ...
  // Ambient companion: one line per card, fetched once. Failure is
  // silent by design — the card is complete without it.
  const [whisper, setWhisper] = useState<string | null>(null);
  useEffect(() => {
    let live = true;
    fetchWhisper(cfg.id).then((w) => { if (live) setWhisper(w?.text || null); });
    return () => { live = false; };
  }, [cfg.id]);
