...
def dimension_stats() -> dict:
    ...
    # Quiet dimensions blend in here (registry, not branches): the 9 above
    # are untouched; spending/social/substance/reading/medication append.
    from superapp.experience import quiet as _quiet
    for _name, _build in _quiet.QUIET_DIMS.items():
        try:
            core[_name] = _build()
        except Exception:
            core[_name] = {'unit': '', 'target': None,
                           'lower_better': False, 'today': None,
                           'series': [], 'week': {}, 'list': []}
    return core
