...
import json
_VITALS_DIR = store.HERMES_DIR / 'integrations'
...
def ingest_sleep(sessions: list) -> int:
    ...
    by_date = {e.get('date'): e for e in store.load_sleep()}
    n = 0
    for s in sessions:
        date, hours = (s.get('date'), s.get('hours'))
        if not date or hours is None:
            continue
        entry = {'date': date, 'hours': float(hours), 'quality': s.get('quality'), 'source': s.get('source', 'wearable')}
        for k in ('deep_min', 'rem_min', 'light_min', 'awake_min'):
            if s.get(k) is not None:
                entry[k] = s[k]
        by_date[date] = entry
        n += 1
    store.save_sleep(list(by_date.values()))
    return n

def ingest_heart_rate(samples: list) -> int:
    store = _read('heart_rate.json')
    store.extend(samples)
    _write('heart_rate.json', store[-2000:])
    return len(samples)
...
def ingest_steps(days: list) -> int:
    ...
    for d in days:
        if d.get('date') is not None and d.get('count') is not None:
            by_date[d['date']] = {'date': d['date'], 'count': int(d['count']), 'source': d.get('source', 'wearable')}
            n += 1
